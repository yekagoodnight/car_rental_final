		...
		<template v-if="showFlag">
			...
			<el-card shadow="hover" class="search-card" :style='{"border":"none","padding":"20px","margin":"0 0 25px","borderRadius":"15px","boxShadow":"0 4px 18px 0 rgba(0, 0, 0, 0.1)","background":"linear-gradient(to right, #ffffff, #f8f9fd)","width":"100%"}'>
				<div class="search-title" style="margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid #ebeef5;">
					...
					<span style="font-size: 16px; color: #333; font-weight: 500;">故障上报搜索</span>
				</div>
			...
			<el-card shadow="hover" :style='{"border":"none","padding":"20px","borderRadius":"15px","boxShadow":"0 4px 18px 0 rgba(0, 0, 0, 0.1)","background":"#fff","width":"100%"}'>
				<div class="table-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid #ebeef5;">
					<div class="table-title" style="display: flex; align-items: center;">
						...
						<span style="font-size: 16px; color: #333; font-weight: 500;">故障上报列表</span>
				...
				>
					...
					<el-table-column prop="guzhangshijian" label="故障时间" align="center" width="60" show-overflow-tooltip>
						<template slot-scope="scope">
							<span>{{scope.row.guzhangshijian}}</span>
						</template>
					</el-table-column>
					<el-table-column prop="guzhangleixing" label="类型" align="center" width="50" show-overflow-tooltip>
						<template slot-scope="scope">
							<span>{{scope.row.guzhangleixing}}</span>
						</template>
					</el-table-column>
					<el-table-column prop="guzhangtupian" label="图" align="center" width="32">
						<template slot-scope="scope">
							<div v-if="scope.row.guzhangtupian" class="image-container">
								<el-image
									:src="scope.row.guzhangtupian && scope.row.guzhangtupian.substring(0,4)=='http' ? 
										(scope.row.guzhangtupian.split(',w').length>1 ? scope.row.guzhangtupian : scope.row.guzhangtupian.split(',')[0]) 
										: $base.url+scope.row.guzhangtupian.split(',')[0]"
									style="width:20px;height:20px;border-radius:2px;object-fit:cover;"
								></el-image>
							</div>
						</template>
					</el-table-column>
					<el-table-column prop="guzhangdidian" label="地点" align="center" width="60" show-overflow-tooltip>
						<template slot-scope="scope">
							<span>{{scope.row.guzhangdidian}}</span>
						</template>
					</el-table-column>
					...
					<el-table-column label="操作" align="center" width="90">
						<template slot-scope="scope">
							<el-button type="text" icon="el-icon-view" size="mini" v-if="isAuth('guzhangshangbao','查看')" @click="addOrUpdateHandler(scope.row.id,'info')" class="view-btn">查看</el-button>
							<el-button type="text" icon="el-icon-edit" size="mini" v-if="isAuth('guzhangshangbao','修改') && scope.row.sfsh=='待审核'" @click="addOrUpdateHandler(scope.row.id)" class="edit-btn">修改</el-button>
						</template>
					</el-table-column>
				</el-table>
				<!-- 分页部分 -->
